...
export default function LiquorCounterPage() {
  ...
  const loadQueueAndStats = async (showSkeleton = false) => {
    ...
    try {
      ...
      setError(
        ...
        'Unable to load liquor queue and statistics.'
      )
    } finally {
      if (showSkeleton) {
        setLoading(false)
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (response.data) {
        // Enforce dedicated workspace boundary: only show result if it matches LIQUOR
        if (response.data.bookingType === 'LIQUOR') {
          setSearchResult(response.data)
        } else {
          setSearchError('This booking is registered for the Grocery Counter, not Liquor.')
        }
      } else {
  ...
  // Filter strictly for LIQUOR bookings
  const liquorQueue = queue.filter(b => b.slot?.cardType === 'LIQUOR')

  const liquorTodayQueue = liquorQueue.filter(b => b.bookingDate === todayStr)

  // Liquor Summary Counts (Today)
  ...
  // Now Serving (Today, first waiting customer in liquor queue)
  const liquorServing = liquorTodayQueue.find(b => b.status === 'BOOKED') || null

  // Completed & Cancelled Today
  const completedToday = liquorTodayQueue.filter(b => b.status === 'CHECKED_OUT')
  const cancelledToday = liquorTodayQueue.filter(b => b.status === 'CANCELLED')
      ...
      <Stack spacing={3}>
        <Box>
          <Typography variant="h4" sx={{ fontWeight: 800, color: '#111827', letterSpacing: '-0.02em', mb: 0.5 }}>
            🥃 Liquor Counter Control
          </Typography>
          <Typography variant="body2" color="text.secondary">
            Loading live liquor queue statistics...
          </Typography>
        </Box>
  ...
  return (
    <>
      <Stack spacing={3}>
        <CounterHeader
          title="🥃 Liquor Counter Page"
          description="Process real-time liquor check-ins, scans, and queue states."
        ...
        <CounterSummaryCard
          cardType="LIQUOR"
          total={liquorToday}
          waiting={liquorWaiting}
          checkedIn={liquorCheckedIn}
          completed={liquorCompleted}
          cancelled={liquorCancelled}
        />
        ...
        <Grid container spacing={3}>
          {/* Completed Today */}
          <Grid item xs={12} md={6}>
            <Card id="completed-liquor-card" sx={{ borderRadius: '12px', border: '1px solid #E5E7EB', boxShadow: 'none' }}>
                    ...
                    {completedToday.length === 0 ? (
                      ...
                      completedToday.map((b) => (
                        <TableRow key={b.id} hover>
                          <TableCell sx={{ fontWeight: 700, fontSize: '0.8rem', color: '#E65100' }}>{b.token}</TableCell>
          ...
          {/* Cancelled Today */}
          <Grid item xs={12} md={6}>
            <Card id="cancelled-liquor-card" sx={{ borderRadius: '12px', border: '1px solid #E5E7EB', boxShadow: 'none' }}>
              <Box sx={{ px: 2.5, py: 1.5, borderBottom: '1px solid #E5E7EB', bgcolor: '#F9FAFB' }}>
                <Typography variant="subtitle2" fontWeight={800} color="#111827">
      ...
      >
        <DialogTitle sx={{ fontWeight: 800, color: '#111827', fontSize: '1.1rem' }}>Scan QR Code (Liquor)</DialogTitle>
        <DialogContent>
          <QrScanner
            onScan={async (token) => {
              try {
                ...
                if (booking.slot?.cardType !== 'LIQUOR') {
                  setSearchError('This scanned QR is registered for the Grocery Counter, not Liquor.')
                  setSearchResult(null)
                  return
                }
